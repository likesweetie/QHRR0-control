...
def trajectory_thread_fn(
    ...
    motor_id_traj만 zero 기준으로 0 -> -0.1 rad 코사인 보간
    """
    dt = 1.0 / hz_traj
    ...
    while g_run:
        ...
        with target_lock:
            target_map[motor_id_traj] = -0.0
            v_des_map[motor_id_traj] = 0
...
def main():
    ...
    # per-motor targets / gains
    target_map: Dict[int, float] = {mid: 0.0 for mid in motor_ids}
    v_des_map: Dict[int, float] = {mid: 0.0 for mid in motor_ids}
    kp_map: Dict[int, float] = {mid: 0.0 for mid in motor_ids}
    kd_map: Dict[int, float] = {mid: 0.0 for mid in motor_ids}
    ...
    try:
        # zero
        for motor_id in motor_ids:
            send_mit_zero(sock, motor_id)
            time.sleep(0.01)

        # enter MIT
        for motor_id in motor_ids:
            send_mit_enter(sock, motor_id)
            time.sleep(0.01)
        ...
        while g_run:
            ...
            # -------------------------
            with target_lock:
                q143 = target_map[traj_motor_id]
                dq143 = v_des_map[traj_motor_id]
            ...
            #     f"q_des(0x{traj_motor_id:X})={q143:+.4f} rad  "
            ...
            if did_recv:
                ...
                for motor_id in motor_ids:
                    st = status_map.get(motor_id, None)
                    if st is not None:
                        print(
                            f"  motor_id=0x{motor_id:X} "
                        ...
                        print(f"  motor_id=0x{motor_id:X}  no cached response")
