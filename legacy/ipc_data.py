...
def create_ipc_data(can_ids: Iterable[int]) -> IPCData:
    ids = list(can_ids)
    return IPCData(
        control=SharedDaemonControl.create(),
        motors={can_id: SharedMotorState.create() for can_id in ids},
        imu=SharedIMUState.create(),
        commands={can_id: SharedMotorCommand.create() for can_id in ids},
    )
...
def set_motor_command(
    ...
    *,
    ...
    shared = ipc.commands.get(can_id)
    if shared is None:
        return
