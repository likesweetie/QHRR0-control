...
def run_can_daemon(ipc: Optional[IPCData] = None, app_cfg: Optional[AppConfig] = None) -> IPCData:
    ...
    local_ipc = ipc if ipc is not None else create_ipc_data(MOTOR_IDS)
    ...
    target_map = {mid: 0.0 for mid in MOTOR_IDS}
    v_des_map: Dict[int, float] = {mid: 0.0 for mid in MOTOR_IDS}
    kp_map: Dict[int, float] = {mid: 0.0 for mid in MOTOR_IDS}
    kd_map: Dict[int, float] = {mid: 1.0 for mid in MOTOR_IDS}
    tau_ff_map: Dict[int, float] = {mid: 0.0 for mid in MOTOR_IDS}
    ...
    try:
        ...
        if cfg_src is not None and len(cfg_src.motor_offsets_deg) == len(MOTOR_IDS):
            offsets = [wrap_to_pi(float(v)) for v in cfg_src.motor_offsets_deg]
        for i, motor_id in enumerate(MOTOR_IDS):
            send_mit_zero(motor_sock, motor_id, RAD2DEG * offsets[i])
            time.sleep(0.01)

        for motor_id in MOTOR_IDS:
            send_mit_enter(motor_sock, motor_id)
            time.sleep(0.01)
        ...
        with target_lock:
            for motor_id in MOTOR_IDS:
                target_map[motor_id] = 0.0
                v_des_map[motor_id] = 0.0
                kp_map[motor_id] = 0.0
                kd_map[motor_id] = 0.0
                tau_ff_map[motor_id] = 0.0
        ...
        wall_t = time.time()
        print("init")
        for motor_id, st in latest_status.items():
            print("motor_id")
        ...
        while g_run and local_ipc.control.run.value:
            ...
            if req_zero_seq:
                for i, motor_id in enumerate(MOTOR_IDS):
                    send_mit_zero(motor_sock, motor_id, offsets[i])
                    time.sleep(0.01)
            ...
            with target_lock:
                if local_ipc.control.damping_enabled.value:
                    for motor_id in MOTOR_IDS:
                        shared_cmd = local_ipc.commands[motor_id]
                        target_map[motor_id] = float(shared_cmd.pos_rad.value)
                        v_des_map[motor_id] = float(shared_cmd.vel_radps.value)
                        kp_map[motor_id] = float(shared_cmd.kp.value)
                        kd_map[motor_id] = float(shared_cmd.kd.value)
                        tau_ff_map[motor_id] = float(shared_cmd.tau_ff.value)
                else:
        ...
        for motor_id in MOTOR_IDS:
            send_mit_exit(motor_sock, motor_id)
            print(f"0x{motor_id:X} down.")
            time.sleep(0.005)
