...
이 문서는 `README.md`를 근거로 사용하지 않고, 현재 코드와 `app_config/*.yaml`에서 확인한 내용만 정리한다. `third_party/`는 handoff 분석 대상에서 제외한다.

## 한눈에 보는 요약

이 프로젝트의 핵심 의도는 **CAN, SHM, 제어 루프, 대시보드, 시뮬레이션을 명확한 런타임 경계로 분리**하는 것이다. 최상위 실행점은 `python3 -m robot_controller.main --config app_config/robot_controller.yaml`이며, `RobotController`가 SHM을 만들고 `ProcessSupervisor`가 하위 프로세스를 시작/종료한다.

현재 확인된 기본 구성은 `vcan0` 대상이다. 실제 CAN interface, actuator CAN ID, SHM 이름, policy path는 모두 `app_config/`에서 읽는다. **motor_id 개념은 런타임 문서 기준에서 사용하지 않는다. 장치 식별은 CAN ID 기준이다.**
...
| 기본 대상 | `app_config/platform.yaml` 기준 `robot.name: qhrr`, CAN interface `vcan0` |
| 제어 경로 | CAN feedback -> `RobotControllerRuntimeIO` -> `qhrr_control_state` SHM -> `task_controller` -> `qhrr_mit_command` SHM -> `RobotController` -> CAN daemon -> SocketCAN |
...
| 범위 | 포함 |
| --- | --- |
| Robot controller | SHM 생성/해제, CAN daemon 연결, IMU request, actuator bringup/shutdown, MIT command validation/송신 |
| Process supervisor | `can_daemon`, `aux_reader`, `task_controller`, `dashboard` 프로세스 시작/종료, pidfile/log 관리 |
...
대시보드는 `app_config/dashboard.yaml` 기준 `http://127.0.0.1:8000`에서 뜬다. `dashboard` 프로세스는 `ProcessSupervisor`가 자동으로 실행한다.
...
| 확인 | 항목 |
| --- | --- |
| [ ] | **`app_config/platform.yaml can.interface`가 실제 CAN interface인지 확인** |
| [ ] | **`app_config/platform.yaml actuators[].can_id`가 실제 actuator CAN ID와 일치하는지 확인** |
| [ ] | **`app_config/robot_controller.yaml can.motors.enter_on_start`가 의도된 bringup 동작인지 확인** |
| [ ] | **`can.command_timeout_s: 0.05`가 실제 feedback 주기와 맞는지 확인** |
| [ ] | **`can.mit_limits`가 firmware MIT packing limit와 일치하는지 확인** |
...
| 위험 | 이유 |
| --- | --- |
| **실제 hardware mode가 별도 flag로 분리되어 있지 않음** | `platform.can.interface` 변경만으로 실제 SocketCAN에 붙을 수 있다. |
| **`enter_on_start: true`이면 controller start 중 actuator enable frame이 전송됨** | 실제 로봇에서는 실행 전 물리 안전 절차가 필요하다. |
| **MIT command batch가 stale/missing이면 damping command를 1회 전송함** | 반복 damping hold가 아니라 reason별 1회 전송이다. recovery는 fresh valid batch 수신이다. |
| **CAN feedback stale이 전역 fault state로 자동 전환되는 구현은 확인되지 않음** | actuator/IMU comm status는 SHM에 표시되지만 global emergency stop은 확인되지 않았다. |
...
| policy 검증 | TODO(owner): ONNX policy output range와 `can.mit_limits`의 관계 검증 |
