...
근거 파일: `robot_controller/utils/shm_manager.py`, `robot_controller/utils/shm_command_router.py`, `robot_controller/core/robot_state_shm.py`, `robot_controller/runtime_io.py`, `robot_controller/process/task_controller/shm_io.py`, `app_config/platform.yaml`, `app_config/robot_controller.yaml`.

## Shared Memory 목록

| SHM name | Owner creates | Writer | Reader | Size/source | Frequency/source |
| --- | --- | --- | --- | --- | --- |
| `qhrr_mit_command` | `ShmManager.create_all()` | `task_controller` via `ShmMitCommandWriter` | `RobotController` via `ShmMitCommandRouter` | header + N targets | task default 50 Hz |
| `qhrr_aux_command` | `ShmManager.create_all()` | `aux_reader` | `task_controller` | `4096` bytes | event-driven; config `publish_hz: 500` is stored but writer publishes on joystick events |
| `qhrr_control_state` | `ShmManager.create_all()` | `RobotControllerRuntimeIO` | `task_controller`, dashboard | `16384` bytes | `500` Hz |
| `qhrr_dashboard_state` | `ShmManager.create_all()` | `RobotControllerRuntimeIO` | dashboard | `65536` bytes | `10` Hz |
...
| startup | `create_all()`로 네 segment 생성 |
...
| `processes` | mapping | `ProcessSupervisor.status()` |
