...
Features:
  ...
  - SPG/MIT actuator decode:
      0x140 + motor_id, usually 0x141..0x14C for 12 motors
      0xC0 MIT status
      0xC1 enter ACK
      0xC2 exit ACK
      0xC3 set-zero ACK
...
def update_spg_state(
    ...
    first_can_id = 0x140 + motor_id_base
    last_can_id = first_can_id + motor_count - 1

    if not (first_can_id <= can_id <= last_can_id):
        return

    motor_id = can_id - 0x140
    motor = state.motors.get(motor_id)
    if motor is None:
        motor = SPGMotorState(motor_id=motor_id, can_id=can_id)
...
def handle_rx_frame(
    ...
    update_spg_state(
        ...
        data=data,
        now=now,
        motor_id_base=args.motor_id_base,
...
def render_dashboard(stdscr, args: argparse.Namespace, state: MonitorState) -> None:
    ...
    row = write_line(
        stdscr,
        row,
        "  ID  CANID  Count  Age       Last              En   Temp  Iq[A]    Speed[dps]  Pos[rad]   Raw",
    )

    for motor_id in range(args.motor_id_base, args.motor_id_base + args.motor_count):
        ...
        if motor is None:
            row = write_line(
                stdscr,
                row,
                f"  {motor_id:02d}  {can_id:03X}  {0:5d}  {'never':>8}  {'-':16s}  {'-':>3}  "
                f"{'-':>4}  {'-':>7}  {'-':>10}  {'-':>8}  -",
            )
            continue
        ...
        row = write_line(
            stdscr,
            row,
            f"  {motor_id:02d}  {can_id:03X}  {motor.rx_count:5d}  {fmt_age(motor.last_t, now):>8}  "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="First SPG motor id. Default: 1 -> first CAN ID 0x141",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Number of SPG motors to display. Default: 12",
    )
