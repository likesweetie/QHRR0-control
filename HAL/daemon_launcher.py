...
from can_daemon import MOTOR_IDS, run_can_daemon
...
def monitor_process_entry(
    ...
    while ipc.control.run.value:
        ...
        for motor_id in motor_ids:
            motor = ipc.motors[motor_id]
            motor_s = motor.snapshot()
            print(
                f"  motor=0x{motor_id:X} temp={motor_s['temp_c']:3.0f}C "
...
def wait_for_daemon_ready(ipc: IPCData, motor_ids: Iterable[int], timeout_s: float = 5.0) -> bool:
    ...
    while time.time() < deadline and ipc.control.run.value:
        ...
        all_fresh = True
        for motor_id in motor_ids:
            updated_at = float(ipc.motors[motor_id].updated_at.value)
            print(ipc.motors[motor_id].updated_at.value)
...
def main() -> None:
    ...
    if app_cfg.launch.enable_monitor:
        monitor_proc = mp.Process(
            ...
            args=(ipc, MOTOR_IDS, app_cfg.launch.monitor_period_s, prompt_done_event),
    ...
    if wait_for_daemon_ready(ipc, MOTOR_IDS, timeout_s=5.0):
        run_task_controller(
            ipc,
