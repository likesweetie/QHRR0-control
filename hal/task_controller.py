...
def _read_motor_angles(ipc: IPCData, motor_ids: Iterable[int], timeout_s: float = 5.0) -> Dict[int, float]:
    ...
    while time.time() < deadline and RUN:
        ...
        for motor_id in mids:
            m = ipc.motors[motor_id]
            updated = float(m.updated_at.value)
            if updated <= 0.0 or (now - updated) > 1.0:
                result.clear()
                break
            result[motor_id] = float(m.enc_rad.value)
    ...
    raise TimeoutError("IPC motor state is not ready")
...
def _write_command(
    ...
    for motor_id, pos in targets.items():
        set_motor_command(
            ipc,
            motor_id,
            pos_rad=pos,
...
def run_task_controller(
    ...
    for motor_id in mids:
        print(f"  motor=0x{motor_id:X} q={q_preview[motor_id]:+.4f}")
